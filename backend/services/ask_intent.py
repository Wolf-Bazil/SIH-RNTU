...
def classify(question: str, place_found: bool) -> Tuple[Intent, Optional[str]]:
    ...
    if not in_domain and not place_found:
...
_geo_cache: Dict[str, Tuple[float, Optional[Dict[str, Any]]]] = {}
...
async def _geocode_one(client: httpx.AsyncClient, name: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        if not results:
            return None
        hit = results[0]
        if hit.get("latitude") is None or hit.get("longitude") is None:
            return None
        return hit
...
async def extract_place(question: str) -> Dict[str, Any]:
    ...
    async with _geo_lock:
        ...
        cached = {c: _geo_cache[c][1] for c in candidates if c in _geo_cache}
        ...
        try:
            async with httpx.AsyncClient(timeout=4.0) as client:
                results = await asyncio.gather(
                    *(_geocode_one(client, c) for c in pending),
